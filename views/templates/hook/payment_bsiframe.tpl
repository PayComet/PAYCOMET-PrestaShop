    ...
    *}
        ...
        {/if}
            ...
            <div id="saved_cards" style="display:none">
                            ...
                            {section name=card loop=$saved_card }
                                {if ($saved_card[card].url=="0")}
                                    {if ($newpage_payment==2)}
                                        <option value='{$paytpv_iframe|escape:'htmlall':'UTF-8':FALSE}'>{l s='NEW CARD' mod='paytpv'}</option>
                                    {else}
                                        <option value='0'>{l s='NEW CARD' mod='paytpv'}</option>
                                    {/if}
                                {else}
                                    <option value='{$saved_card[card].url|escape:'htmlall':'UTF-8':FALSE}'>{$saved_card[card].CC|escape:'htmlall':'UTF-8':FALSE} ({$saved_card[card].BRAND|escape:'htmlall':'UTF-8':FALSE}){if ($saved_card[card].CARD_DESC!="")} - {$saved_card[card].CARD_DESC|escape:'htmlall':'UTF-8':FALSE}{/if}</option>
                                {/if}
                            {/section}
                        </select>
                    </div>
                ...
                <div id="button_directpay">
                    ...
                    {/if}
                    <img id='clockwait' style="display:none" src="{$this_path|escape:'htmlall':'UTF-8':FALSE}views/img/clockpayblue.gif"></img>
                </div>
                {/if}
            ...
            {if (!$disableoffersavecard==1)}
                <div id="storingStep" class="alert alert-info {if (sizeof($saved_card))>1}hidden{/if}" style="clear:left;">                   
                    <p class="checkbox"><label for="savecard" class="checkbox"><input type="checkbox" name="savecard" id="savecard" onChange="saveOrderInfoJQ(0)">{l s='Save card for future purchases' mod='paytpv'}.<span class="paytpv-pci">{l s='Card data is protected by the Payment Card Industry Data Security Standard (PCI DSS)' mod='paytpv'}.</span></label></p>
                </div>
            {/if}
            ...
            <div class="payment_module paytpv_iframe" style="display:none">
                ...
                {if ($newpage_payment<2)}
                    ...
                    {if ($paytpv_integration==0)}
                        ...
                        <iframe id="paytpv_iframe" src="{$paytpv_iframe|escape:'htmlall':'UTF-8':FALSE}" name="paytpv" style="width: 670px; border-top-width: 0px; border-right-width: 0px; border-bottom-width: 0px; border-left-width: 0px; border-style: initial; border-color: initial; border-image: initial; height: {$iframe_height|escape:'htmlall':'UTF-8'}px; " marginheight="0" marginwidth="0" scrolling="no"></iframe>
                    {else}
                        <form action="{$paytpv_jetid_url|escape:'htmlall':'UTF-8':FALSE}" method="POST" class="paytpv_jet" id="paytpvPaymentForm" onsubmit="return takingOff();" style="clear:left;">
                        <ul>
                            <li>
    ...
    <div style="display: none;">
        <div id="conditions" style="overflow:auto;">
            ...
            <p>
                {l s='This business does not store or transmit credit card or debit card data. Data is sent over an encrypted and secure channel to the PAYCOMET platform.' mod='paytpv'}
            </p>
            <p>
