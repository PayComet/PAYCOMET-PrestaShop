...
class PaytpvOrderInfo extends ObjectModel
{
    ...
    public static function saveOrderInfo(
        ...
        $sql = 'DELETE FROM '. _DB_PREFIX_ .'paytpv_order_info where id_customer = '.pSQL($id_customer)
    ...
    {
        $sql = 'select * from ' . _DB_PREFIX_ .'paytpv_order_info where id_customer = '.pSQL($id_customer)
        .' and id_cart="'.pSQL($id_cart).'"';
        $result = Db::getInstance()->getRow($sql);

        // Si no hay datos los almacenamos segun la configuración: Disable Offer Card != SI y Remember Card
        // (Unselected) != SI Por defecto se guarda la tarjeta si está por defecto seleccionado.
