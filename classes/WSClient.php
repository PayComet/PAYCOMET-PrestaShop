...
class WSClient
{
    ...
    public function __construct(
        ...
        $useCURL = Tools::getIsset('usecurl') ? Tools::getValue('usecurl') : '0';
    ...
    public function executePurchase(
        ...
        $DS_MERCHANT_MERCHANTSIGNATURE = hash(
            'sha512',
            $DS_MERCHANT_MERCHANTCODE . $DS_IDUSER . $DS_TOKEN_USER . $DS_MERCHANT_TERMINAL . $DS_MERCHANT_AMOUNT
            .$DS_MERCHANT_ORDER . $this->config[ 'pass' ]
        );
    ...
    public function infoUser($idUser, $tokeUser)
    {
        ...
        $DS_MERCHANT_MERCHANTSIGNATURE = hash(
            'sha512',
            $DS_MERCHANT_MERCHANTCODE . $DS_IDUSER . $DS_TOKEN_USER . $DS_MERCHANT_TERMINAL . $this->config[ 'pass' ]
        );
    ...
    public function removeUser($idUser, $tokeUser)
    {
        ...
        $this->writeLog("Petición remove_user:\n".print_r($p, true));

        $res = $this->client->call('remove_user', $p, '', '', false, true);

        $this->writeLog("Respuesta remove_user:\n".print_r($res, true));
    ...
    public function executeRefund($idUser, $tokeUser, $order, $currency, $authcode, $amount)
    {
        ...
        $DS_MERCHANT_MERCHANTSIGNATURE = hash(
            'sha512',
            $DS_MERCHANT_MERCHANTCODE . $DS_IDUSER . $DS_TOKEN_USER . $DS_MERCHANT_TERMINAL . $DS_MERCHANT_AUTHCODE
            .$DS_MERCHANT_ORDER . $this->config[ 'pass' ]
        );
    ...
    public function addUserToken($token)
    {
        ...
        $DS_MERCHANT_MERCHANTSIGNATURE = hash(
            'sha512',
            $DS_MERCHANT_MERCHANTCODE.$token.$DS_JETID.$DS_MERCHANT_TERMINAL.$this->config[ 'pass' ]
        );
