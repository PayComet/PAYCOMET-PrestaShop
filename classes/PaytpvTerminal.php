...
class PaytpvTerminal extends ObjectModel
{
    ...
    public static function addTerminal(
        ...
        $id_shop = Context::getContext()->shop->id;
        $sql = 'INSERT INTO '. _DB_PREFIX_ .'paytpv_terminal (id,id_shop,idterminal,idterminal_ns,password,password_ns
        ,jetid,jetid_ns,currency_iso_code,terminales,tdfirst,tdmin) VALUES('.(int)$id.','.(int)$id_shop.
        ','.$idterminal.','.$idterminal_ns.',"'.pSQL($password).'","'.pSQL($password_ns).'","'.pSQL($jetid).
        '","'.pSQL($jetid_ns).'","'.pSQL($currency_iso_code).'",'.(int)$terminales.','.(int)$tdfirst.
        ','.(float)$tdmin.')';
    ...
    public static function getTerminalByIdTerminal($idterminal)
    {
        ...
        $sql = 'select * from ' . _DB_PREFIX_ .'paytpv_terminal where (idterminal='.(int)$idterminal.
        ' or idterminal_ns='.(int)$idterminal .') and id_shop=' . (int)$id_shop;
