...
class PaytpvTerminal extends ObjectModel
{
    ...
    public static function addTerminal(
        ...
        ,jetid,jetid_ns,currency_iso_code,terminales,tdfirst,tdmin) VALUES('.$id.','.$id_shop.','.$idterminal.','
        .$idterminal_ns.',"'.$password.'","'.$password_ns.'","'.$jetid.'","'.$jetid_ns.'","'.$currency_iso_code.'",'
        .$terminales.','.$tdfirst.','.$tdmin.')';
    ...
    {
        $id_shop = Context::getContext()->shop->id;
        $sql = 'select * from ' . _DB_PREFIX_ .'paytpv_terminal where (idterminal='.$idterminal.' or idterminal_ns='
        .$idterminal .') and id_shop=' . $id_shop;
