...
class PaytpvSuscription extends ObjectModel
{
    ...
    public static function saveSuscription(
        ...
        if (empty($result) === true) {
            ...
            `paytpv_tokenuser`,`periodicity`,`cycles`,`price`,`date`) VALUES('.(int)$id_customer.','.(int)$id_order
            .','.(int)$paytpv_iduser.',"'.pSQL($paytpv_tokenuser).'",'.(int)$periodicity.','.(int)$cycles.','
            .(float)$importe.',"'.pSQL(date('Y-m-d H:i:s')).'")';
    ...
    public static function getSuscriptionOrderPayments($id_order)
    {
        ...
        .pSQL($id_order) . ' where ps.id_order = '. (int)$id_order. ' group by ps.id_suscription order by
        ps.date desc';
        $result = Db::getInstance()->getRow($sql);
        ...
        if (empty($result)) {
            ...
            po.id_suscription where po.id_order = '. (int)$id_order. ' group by ps.id_suscription order by
            ps.date desc';
            $result = Db::getInstance()->getRow($sql);
        }
        return $result;
