...
class Paytpv extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        try {
            if (!isset($this->apikey) || !isset($this->clientcode) || $this->clientcode==""
            || !PaytpvTerminal::existTerminal()) {
    ...
    public function hookDisplayPayment($params)
    {
        ...
        // Pago en nueva pagina fullscreen si no tiene tarjetas almacenadas
            ...
            $this->context->smarty->assign(
                'paytpv_module',
                Context::getContext()->link->getModuleLink($this->name, 'actions', array(), $ssl)
            );
        ...
        switch ($methodId) {
            case 33: // Instant Credit
                if ($this->paytpv_apm_instant_credit_simulador) {
                    ...
                    $html_code .= '<script src="'. $this->getICSimulatorUrl() .'" charset="UTF-8"></script>';
                }
                break;
            default:
                break;
